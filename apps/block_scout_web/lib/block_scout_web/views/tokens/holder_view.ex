defmodule BlockScoutWeb.Tokens.HolderView do
  ...
  ## Examples
    ...
    iex> token = build(:token, type: "ZEN-20", decimals: Decimal.new(2))
    iex> BlockScoutWeb.Tokens.HolderView.format_token_balance_value(100000, nil, token)
    "1,000"

    iex> token = build(:token, type: "ZEN-721")
    iex> BlockScoutWeb.Tokens.HolderView.format_token_balance_value(1, nil, token)
    1

  """
  def format_token_balance_value(value, _id, %Token{type: "ZEN-20", decimals: decimals}) do
    format_according_to_decimals(value, decimals)
  end

  def format_token_balance_value(value, id, %Token{type: "ZEN-1155", decimals: decimals}) do
