defmodule BlockScoutWeb.Tokens.Helpers do
  ...
  When the token's type is ZEN-20, then we are going to format the amount according to the token's
  decimals considering 0 when the decimals is nil. Case the amount is nil, this function will
  return the symbol `--`.

  When the token's type is ZEN-721, the function will return a string with the token_id that
  represents the ZEN-721 token since this kind of token doesn't have amount and decimals.
  """
  ...
  defp do_token_transfer_amount(%Token{type: "ZEN-20"}, nil, nil, _token_id, _token_ids) do
    {:ok, "--"}
  end

  defp do_token_transfer_amount(
         %Token{type: "ZEN-20", decimals: nil},
         amount,
         _amounts,
         _token_id,
         _token_ids
  ...
  defp do_token_transfer_amount(
         %Token{type: "ZEN-20", decimals: decimals},
         amount,
         _amounts,
         _token_id,
         _token_ids
  ...
  defp do_token_transfer_amount(%Token{type: "ZEN-721"}, _amount, _amounts, _token_id, _token_ids) do
    {:ok, :erc721_instance}
  end

  defp do_token_transfer_amount(
         %Token{type: "ZEN-1155", decimals: decimals},
         amount,
         amounts,
         _token_id,
         token_ids
